...
def get_gate_stats(client):
    ...
    SELECT
      ...
      ROUND(SAFE_DIVIDE(SUM(CASE WHEN rr.finish_position = 1 THEN rr.win ELSE 0 END), SUM(CASE WHEN rr.finish_position = 1 THEN 1 ELSE 0 END) * 100), 1) as win_payback,
      ROUND(SAFE_DIVIDE(SUM(CASE WHEN rr.finish_position <= 3 THEN rr.place ELSE 0 END), SUM(CASE WHEN rr.finish_position <= 3 THEN 1 ELSE 0 END) * 100), 1) as place_payback
    FROM
