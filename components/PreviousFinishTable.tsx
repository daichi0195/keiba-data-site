...
import styles from './PreviousFinishTable.module.css';

type PreviousFinishRow = {
  rank: number;
...
type Props = {
  title: string;
  data: PreviousFinishRow[];
};

export default function PreviousFinishTable({ title, data }: Props) {
  ...
  // 前走着順バッジのクラス名を取得（全てグレー統一）
  const getFinishBadgeClass = () => {
    return styles.finishBadge;
  };
      ...
      <div className={styles.tableContainer}>
        <div className={styles.tableScroll} ref={scrollRef}>
          <table className={styles.table}>
            ...
            <tbody>
              {data.map((row, index) => {
                const badgeClass = getFinishBadgeClass();

                return (
                  <tr
                    key={row.finish_category}
                    className={index % 2 === 0 ? styles.rowEven : styles.rowOdd}
                  >
                    <td className={styles.finishCol}>
                      <span className={badgeClass}>{row.finish_category}</span>
                    </td>
