...
import styles from './TrackConditionTable.module.css';

type TrackConditionRow = {
  name: string;
  surface: string;
...
type Props = {
  title: string;
  data: TrackConditionRow[];
};

export default function TrackConditionTable({ title, data }: Props) {
        ...
        <div className={styles.tableScroll} ref={scrollRef}>
          <table className={styles.table}>
            ...
            <tbody>
              {data.map((row, index) => (
                <tr
                  key={`${row.surface}-${row.condition_label}`}
