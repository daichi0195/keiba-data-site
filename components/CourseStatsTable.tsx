...
import styles from './CourseStatsTable.module.css';
...
export default function CourseStatsTable({ title, data }: Props) {
                        ...
                        {racecourse.courses.map((course, index) => (
                          ...
                          >
                            <td className={styles.courseCol} suppressHydrationWarning>
                              <Link href={course.link} className={styles.courseLink} suppressHydrationWarning>
                                {formatCourseName(course.name, racecourse.racecourse_ja)}
                              </Link>
                            </td>
                            <td className={styles.scrollCol}>
                              <span className={isHighlight(course.races, maxValues.maxRaces) ? styles.highlight : ''}>
                                {formatNumber(course.races)}
                              </span>
                            </td>
                            <td className={styles.scrollCol}>
                              <span className={isHighlight(course.wins, maxValues.maxWins) ? styles.highlight : ''}>
                                {course.wins}
                              </span>
                            </td>
                            <td className={styles.scrollCol}>
                              <span className={isHighlight(course.places_2, maxValues.maxPlaces2) ? styles.highlight : ''}>
                                {course.places_2}
                              </span>
                            </td>
                            <td className={styles.scrollCol}>
                              <span className={isHighlight(course.places_3, maxValues.maxPlaces3) ? styles.highlight : ''}>
                                {course.places_3}
                              </span>
                            </td>
                            <td className={styles.scrollCol}>
                              <span className={isHighlight(course.win_rate ?? 0, maxValues.maxWinRate) ? styles.highlight : ''}>
                                {(course.win_rate ?? 0).toFixed(1)}%
                              </span>
                            </td>
                            <td className={styles.scrollCol}>
                              <span className={isHighlight(course.quinella_rate ?? 0, maxValues.maxQuinellaRate) ? styles.highlight : ''}>
                                {(course.quinella_rate ?? 0).toFixed(1)}%
                              </span>
                            </td>
                            <td className={styles.scrollCol}>
                              <span className={isHighlight(course.place_rate ?? 0, maxValues.maxPlaceRate) ? styles.highlight : ''}>
                                {(course.place_rate ?? 0).toFixed(1)}%
                              </span>
                            </td>
                            <td className={styles.scrollCol}>
                              <span className={isHighlight(course.win_payback ?? 0, maxValues.maxWinPayback) ? styles.highlight : ''}>
