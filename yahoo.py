...
def add_report_last7days(session: requests.Session, source: str, token: str,
        ...
        def _add():
            ...
            try:
                ...
                job.job_id = _with_retry(
                    add_report_last7days, max_retries,
                    new_session(), job.source, token, base_account_id, job.account_id,
                    job.fields, job.lang, job.report_type
                )
                return job
            finally:
...
def main():
    global VERBOSE
    ap = argparse.ArgumentParser(description="Yahoo Search + Display v17 → 直近7日を取得し、エンティティ別に Google 風の型で Parquet 保存")
