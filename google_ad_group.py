"""
Google Ads API - 広告グループ単位レポート取得スクリプト

このスクリプトは、Google Ads APIを使用して広告グループ単位のパフォーマンスデータを取得し、
CSV ファイルに出力します。
"""
...
class GoogleAdsAdGroupReport:
    """Google Ads 広告グループ単位レポート取得クラス"""
    ...
    def get_ad_group_metrics(self, customer_id, start_date, end_date):
        """
        広告グループ単位のメトリクスを取得
        ...
        Returns:
            pd.DataFrame: 広告グループ単位のメトリクスデータ
        """
        ga_service = self.client.get_service("GoogleAdsService")

        # クエリの構築（主要メトリクス + インプレッションシェア系 + 上位表示率系）
        query = f"""
            SELECT
                segments.date,
                customer.id,
        ...
        try:
            ...
            # SELECT句の順番に従ってメトリクスを追加（主要メトリクスのみ）
            ...
            column_order = ['date', 'customer_id', 'campaign_id', 'ad_group_id', 'ad_group_name'] + metrics_in_select_order
...
def main():
    ...
    # ===== CSV出力パスの設定 =====
    output_dir = "google/ad_group"
    os.makedirs(output_dir, exist_ok=True)

    # タイムスタンプをファイル名に含める
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join(output_dir, f"ad_group_report_{timestamp}.csv")

    # ===== データ取得 =====
    # レポート取得インスタンスを作成（環境変数から認証情報を取得）
    try:
        report = GoogleAdsAdGroupReport()
    ...
    for i, customer_id in enumerate(CUSTOMER_IDS, 1):
        ...
        try:
            df = report.get_ad_group_metrics(customer_id, START_DATE, END_DATE)
            if len(df) > 0:
                all_dfs.append(df)
