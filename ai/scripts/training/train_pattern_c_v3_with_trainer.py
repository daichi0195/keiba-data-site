...
def load_data_from_bigquery():
    ...
    print("=" * 100)
    print("📊 BigQueryからデータを読み込み中（パターンC v3: 調教師統計追加）...")
    print("=" * 100)
...
def prepare_features(df):
    """特徴量を準備"""
    print("\n" + "=" * 100)
    print("🔧 特徴量エンジニアリング（パターンC v3: 調教師統計追加）")
    print("=" * 100)
    ...
    print(f"\n【追加した特徴量】")
    print(f"  1. trainer_place_rate_surface_distance（調教師複勝率）")
    print(f"  2. trainer_win_rate_surface_distance（調教師勝率）")
...
def train_and_evaluate(df):
    ...
    # モデル保存
    model.save_model('../../models/model_pattern_c_v3_with_trainer.txt')
    with open('../../models/model_pattern_c_v3_with_trainer.pkl', 'wb') as f:
        pickle.dump(model, f)

    print(f"\n💾 モデル保存完了")
    print(f"   - models/model_pattern_c_v3_with_trainer.txt")
    print(f"   - models/model_pattern_c_v3_with_trainer.pkl")

    # 特徴量重要度をCSV保存
    feature_importance.to_csv('../../data/evaluations/feature_importance_pattern_c_v3.csv', index=False, encoding='utf-8-sig')
    print(f"   - data/evaluations/feature_importance_pattern_c_v3.csv")
...
def main():
    ...
    print("✅ パターンC v3訓練完了")
    print(f"   特徴量数: 32個")
    print(f"   追加: trainer_place_rate, trainer_win_rate")
