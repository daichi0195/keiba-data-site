...
# -*- coding: utf-8 -*-
"""
パターンC v2: 騎手複勝率 + 勝率の両方（最適化版）

変更点:
- 重要度0%の特徴量を削除
  1. is_consecutive_race (0.00%)
  2. going_encoded (0.00%)
...
def load_data_from_bigquery():
    ...
    print("=" * 100)
    print("📊 BigQueryからデータを読み込み中（パターンC v2: 最適化版）...")
    print("=" * 100)
...
def prepare_features(df):
    """特徴量を準備"""
    print("\n" + "=" * 100)
    print("🔧 特徴量エンジニアリング（パターンC v2: 最適化版）")
    print("=" * 100)
    ...
    # 特徴量リスト（30個）
    # 削除: is_consecutive_race, going_encoded
    features = [
        # 脚質（4個）
        ...
        # 休養関連（1個）← is_consecutive_raceを削除
        'rest_period_category'
    ]

    print(f"\n✅ 使用する特徴量: {len(features)}個")
    print(f"\n【削除した特徴量（重要度0%）】")
    print(f"  1. is_consecutive_race (0.00%)")
    print(f"  2. going_encoded (0.00%)")
...
def train_and_evaluate(df):
    ...
    # モデル保存
    model.save_model('../../models/model_pattern_c_v2_optimized.txt')
    with open('../../models/model_pattern_c_v2_optimized.pkl', 'wb') as f:
        pickle.dump(model, f)

    print(f"\n💾 モデル保存完了")
    print(f"   - models/model_pattern_c_v2_optimized.txt")
    print(f"   - models/model_pattern_c_v2_optimized.pkl")

    # 特徴量重要度をCSV保存
    feature_importance.to_csv('../../data/evaluations/feature_importance_pattern_c_v2.csv', index=False, encoding='utf-8-sig')
    print(f"   - data/evaluations/feature_importance_pattern_c_v2.csv")
...
def main():
    print("\n" + "=" * 100)
    print("🐎 パターンC v2: 重要度0%特徴量削除（最適化版）")
    print("=" * 100)
    ...
    print("\n" + "=" * 100)
    print("✅ パターンC v2訓練完了")
    print(f"   特徴量数: 30個（32個 → 30個）")
    print(f"   削除: is_consecutive_race, going_encoded")
