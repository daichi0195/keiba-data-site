-- running_style_combined_win_rate 特徴量の追加
...
-- 脚質別勝率の計算（全期間のデータを使用、ただし予測対象レースより前のみ）
