...
export async function getSireDataFromGCS(sireId: string | number) {
  ...
  try {
    ...
    console.log('✅ Sire data loaded from GCS');
    console.log('  - Sire:', data.name || '(unknown)');
    ...
    console.error('❌ Failed to fetch sire data:', error);
    throw error;
  }
}
